/**
 * FloatingButton - Floating action button component for task management
 * 
 * Features:
 * - New task creation
 * - Batch delete selected tasks
 * - Batch update task status
 * - Dynamic button display based on selection
 */

import React, { useState } from 'react';
import { FloatButton, Modal, Select, Space, message } from 'antd';
import { 
  PlusOutlined, 
  DeleteOutlined, 
  EditOutlined,
  SettingOutlined,
  BulbOutlined
} from '@ant-design/icons';
import { TaskStatus, STATUS_CONFIG } from '../types/task';

const { Option } = Select;

/**
 * FloatingButton Props
 * 
 * @property {string[]} selectedTaskIds - Currently selected task IDs
 * @property {() => void} onNewTask - Callback for creating new task
 * @property {(taskIds: string[]) => void} onBatchDelete - Callback for batch delete
 * @property {(taskIds: string[], status: TaskStatus) => void} onBatchStatusUpdate - Callback for batch status update
 * @property {() => void} onClearSelection - Callback for clearing selection
 */
interface FloatingButtonProps {
  selectedTaskIds: string[];
  onNewTask: () => void;
  onBatchDelete: (taskIds: string[]) => void;
  onBatchStatusUpdate: (taskIds: string[], status: TaskStatus) => void;
  onClearSelection: () => void;
}

const FloatingButton: React.FC<FloatingButtonProps> = ({
  selectedTaskIds,
  onNewTask,
  onBatchDelete,
  onBatchStatusUpdate,
  onClearSelection
}) => {
  // Controls visibility of batch status update modal
  const [batchStatusModalVisible, setBatchStatusModalVisible] = useState(false);
  
  // Selected status for batch update
  const [selectedStatus, setSelectedStatus] = useState<TaskStatus | undefined>();

  /**
   * Handle batch delete with confirmation dialog
   * Shows task count and executes delete after confirmation
   */
  const handleBatchDelete = () => {
    Modal.confirm({
      title: '确认批量删除',
      content: `确定要删除选中的 ${selectedTaskIds.length} 个任务吗？此操作无法撤销。`,
      okText: '删除',
      okType: 'danger',
      cancelText: '取消',
      onOk: () => {
        onBatchDelete(selectedTaskIds);
        onClearSelection();
        message.success('任务删除成功');
      }
    });
  };

  /**
   * Open batch status update modal
   */
  const handleBatchStatusUpdate = () => {
    setBatchStatusModalVisible(true);
    setSelectedStatus(undefined);
  };

  /**
   * Confirm and execute batch status update
   * Validates status selection before updating
   */
  const confirmBatchStatusUpdate = () => {
    // Validate status selection
    if (!selectedStatus) {
      message.warning('请选择要更新的状态');
      return;
    }
    
    // Execute batch status update
    onBatchStatusUpdate(selectedTaskIds, selectedStatus);
    // Clear selection state
    onClearSelection();
    // Close modal and reset state
    setBatchStatusModalVisible(false);
    setSelectedStatus(undefined);
    // Display success message
    message.success('任务状态更新成功');
  };

  /**
   * Generate quick action buttons based on selection state
   * Returns batch operation buttons when tasks are selected, hint button otherwise
   */
  const getQuickActions = () => {
    if (selectedTaskIds.length > 0) {
      // Batch operation buttons when tasks are selected
      return [
        {
          icon: <EditOutlined />,
          tooltip: `批量更新状态 (${selectedTaskIds.length}个)`,
          onClick: handleBatchStatusUpdate,
          type: 'primary' as const
        },
        {
          icon: <DeleteOutlined />,
          tooltip: `批量删除 (${selectedTaskIds.length}个)`,
          onClick: handleBatchDelete,
          type: 'default' as const,
          danger: true
        },
        {
          icon: <SettingOutlined />,
          tooltip: '取消选择',
          onClick: onClearSelection,
          type: 'default' as const
        }
      ];
    } else {
      // Hint button when no tasks are selected
      return [
        {
          icon: <BulbOutlined />,
          tooltip: '快捷提示：选择任务后可进行批量操作',
          onClick: () => message.info('选择一个或多个任务来进行批量操作'),
          type: 'default' as const
        }
      ];
    }
  };

  return (
    <>
      {/* Floating button group */}
      <FloatButton.Group
        trigger="hover"
        type="primary"
        style={{ right: 24 }}
        icon={<SettingOutlined />}
        tooltip="操作菜单"
      >
        {/* New task button - always visible */}
        <FloatButton
          icon={<PlusOutlined />}
          tooltip="新建任务"
          onClick={onNewTask}
          type="primary"
        />
        
        {/* Dynamic action buttons based on selection state */}
        {getQuickActions().map((action, index) => (
          <FloatButton
            key={index}
            icon={action.icon}
            tooltip={action.tooltip}
            onClick={action.onClick}
            type={action.type}
            {...(action.danger && { style: { backgroundColor: '#ff4d4f' } })}  // Red background for dangerous operations
          />
        ))}
      </FloatButton.Group>

      {/* Batch status update modal */}
      <Modal
        title="批量更新任务状态"
        open={batchStatusModalVisible}
        onOk={confirmBatchStatusUpdate}
        onCancel={() => {
          setBatchStatusModalVisible(false);
          setSelectedStatus(undefined);
        }}
        okText="更新"
        cancelText="取消"
        width={400}
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <p>将为选中的 <strong>{selectedTaskIds.length}</strong> 个任务更新状态：</p>
          
          {/* Status selector */}
          <Select
            placeholder="请选择新状态"
            value={selectedStatus}
            onChange={setSelectedStatus}
            style={{ width: '100%' }}
            size="large"
          >
            {Object.values(TaskStatus).map(status => (
              <Option key={status} value={status}>
                <Space>
                  {/* Color indicator for status */}
                  <span 
                    style={{ 
                      display: 'inline-block',
                      width: 8,
                      height: 8,
                      borderRadius: '50%',
                      // Color mapping: default(gray), processing(blue), success(green), error(red)
                      backgroundColor: STATUS_CONFIG[status].color === 'default' ? '#d9d9d9' :
                                     STATUS_CONFIG[status].color === 'processing' ? '#1890ff' :
                                     STATUS_CONFIG[status].color === 'success' ? '#52c41a' :
                                     STATUS_CONFIG[status].color === 'error' ? '#ff4d4f' : '#d9d9d9'
                    }} 
                  />
                  {STATUS_CONFIG[status].label}
                </Space>
              </Option>
            ))}
          </Select>
          
          {/* Status description */}
          {selectedStatus && (
            <div style={{ 
              padding: '8px 12px', 
              backgroundColor: '#f6ffed', 
              border: '1px solid #b7eb8f',
              borderRadius: 4,
              fontSize: 12,
              color: '#389e0d'
            }}>
              {STATUS_CONFIG[selectedStatus].description}
            </div>
          )}
        </Space>
      </Modal>
    </>
  );
};

export default FloatingButton;